...
const SortProducts = () => {
  ...
  const onStatusChange = ({ target }) => {
    if (target.name === sortStatus || target.parentNode.name === sortStatus) {
      return;
    }
  ...
  return (
    <div className="col">
      ...
      <button
        name="highToLow"
        className="waves-effect blue accent-1 btn-small"
        onClick={(e) => onStatusChange(e)}
      >
        <i className="material-icons left">sort</i>цена по убыванию
      </button>
      <button
        name="lowToHigh"
        className="waves-effect blue accent-1 btn-small"
        onClick={(e) => onStatusChange(e)}
      >
        <i className="material-icons left revers-sort">sort</i>цена по
        возрастанию
      </button>
    </div>
  );
